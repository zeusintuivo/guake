...
class RootTerminalBox(Gtk.Overlay, TerminalHolder):
    ...
    def _add_search_box(self):
        ...
        self.search_revealer_show_cb_id = self.search_revealer.connect("show", search_revealer_show_cb)
            ...
            while True:
                if self.guake:
                    ...
                    if (
                        ...
                        or self.get_notebook() is not self.guake.notebook_manager.get_current_notebook()
                    ):
                        panes.insert(0, cur)
    ...
    def block_notebook_on_button_press_id(self):
        GObject.signal_handler_block(self.get_notebook(), self.get_notebook().notebook_on_button_press_id)

    def unblock_notebook_on_button_press_id(self):
        GObject.signal_handler_unblock(self.get_notebook(), self.get_notebook().notebook_on_button_press_id)
...
class TerminalBox(Gtk.Box, TerminalHolder):
    ...
    def set_terminal(self, terminal):
        ...
        self.terminal.handler_ids.append(self.terminal.connect("grab-focus", self.on_terminal_focus))
        self.terminal.handler_ids.append(self.terminal.connect("button-press-event", self.on_button_press, None))
        self.terminal.handler_ids.append(self.terminal.connect("child-exited", self.on_terminal_exited))
    ...
    def add_scroll_bar(self):
        ...
        self.terminal.handler_ids.append(self.terminal.connect("scroll-event", self.__scroll_event_cb))
    ...
    def on_button_press(self, target, event, user_data):
        if event.button == 3:
            ...
            if not event.get_state() & Gdk.ModifierType.SHIFT_MASK and Vte.Terminal.do_button_press_event(
                self.terminal, event
            ):
                return True
    ...
    def grab_box_terminal_focus(self, box):
        if isinstance(box, DualTerminalBox):
            try:
                ...
                logger.error("%s Both panes are empty", _fl_two_())
        else:
...
class TabLabelEventBox(Gtk.EventBox):
    def __init__(self, notebook, text, settings):
        ...
        settings.general.bind("tab-close-buttons", self.close_button, "visible", Gio.SettingsBindFlags.GET)
