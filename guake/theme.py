...
def get_resource_dirs(resource):
    ...
    dirs = [
        ...
        for dir in itertools.chain(GLib.get_system_data_dirs(), GUAKE_THEME_DIR, GLib.get_user_data_dir())
    ]
...
def list_all_themes():
    return sorted({x.name for theme_dir in get_resource_dirs("themes") for x in theme_dir.iterdir() if x.is_dir()})


def select_gtk_theme(settings):
    gtk_settings = Gtk.Settings.get_default()
    if settings.general.get_boolean("gtk-use-system-default-theme"):
        logger.debug("%s  Using system default theme", _fl_two_())
        gtk_settings.reset_property("gtk-theme-name")
        gtk_settings.set_property("gtk-application-prefer-dark-theme", False)
        return

    gtk_theme_name = settings.general.get_string("gtk-theme-name")
    logger.debug("%s  Wanted GTK theme: %r", _fl_two_(), gtk_theme_name)
    ...
    logger.debug("%s  Prefer dark theme: %r", _fl_two_(), prefer_dark_theme)
...
def patch_gtk_theme(style_context, settings):
    ...
    logger.debug(lgt, _fl_two_(), theme_name, dark_yes, selected_fg_color, selected_bg_color)
    css_data = dedent(
        f"""
        .custom_tab:checked {{
