...
class CustomCommands:
    ...
    def _load_json(self, file_name):
        logger.info(_file_()+":"+_line_()+" Loading menu json file::: %s", file_name)
        if not os.path.exists(file_name):
            logger.error(_file_()+":"+_line_()+" Custom file does not exit: %s", file_name)
            return None
        try:
    ...
    def build_menu(self):
        if not self.should_load():
            return None
        menu = Gtk.Menu()
        logger.info(_file_()+":"+_line_()+" Loading session json file: %s", self.get_file_path())
