...
if os.path.isdir(themes_folder):
    ...
    for theme_filename in theme_filenames:
        with open(os.path.join(themes_folder, theme_filename),
                  "r",
                  encoding="utf-8") as theme_file:
            try:
                theme_to_load = json.load(theme_file)
                PALETTES = {**PALETTES, **theme_to_load}
                logger.debug("%s:%s Loaded themes %s", _file_(), _line_(),
                             " ".join(theme_to_load.keys()))
            except json.JSONDecodeError:
                logger.debug("%s:%s Unable to load theme from file %s",
                             _file_(), _line_(), theme_file)
else:
    logger.debug("%s:%s Could not find themes folder %s", _file_(), _line_(),
                 themes_folder)
